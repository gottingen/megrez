...
TEST(test_parser, case_encode) {
    // gbk
    {
        ...
        while (std::getline(f, line)) {
            std::vector<std::string> split_vec = turbo::str_split(line, "|");
            std::string sql(turbo::trim_all(split_vec[0]));
            std::string assert_sql(turbo::trim_all(split_vec[1]));
            megrez::SqlParser parser;
            parser.charset = "gbk";
            parser.parse(sql);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            megrez::SelectStmt* select_stmt = (megrez::SelectStmt*)parser.result[0];
...
TEST(test_parser, case_option) {
    //select
    {
        ...
        ASSERT_EQ(0, parser.error);
        ASSERT_EQ(1, parser.result.size());
        ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(megrez::SelectStmt));
        megrez::SelectStmt* select_stmt = (megrez::SelectStmt*)parser.result[0];
    ...
    }
    {
        ...
        megrez::SelectStmt* select_stmt = (megrez::SelectStmt*)parser.result[0];
        ASSERT_EQ(1, select_stmt->fields.size());
        megrez::SelectField* field = select_stmt->fields[0];
        ASSERT_TRUE(field->expr != nullptr);
        megrez::ExprNode* expr = field->expr;
        ASSERT_TRUE(expr->expr_type == megrez::ET_FUNC);
        megrez::FuncExpr* func_expr = (megrez::FuncExpr*)(expr);
...
TEST(test_parser, case_field) {
    // test select_fields
    {
        ...
        ASSERT_EQ(1, select_stmt->fields.size());
        megrez::SelectField* field = select_stmt->fields[0];
        ASSERT_TRUE(field->node_type == megrez::NT_SELECT_FEILD);
        ASSERT_TRUE(field->expr == nullptr); 
        ASSERT_TRUE(field->as_name.value == nullptr); 
        ASSERT_TRUE(field->wild_card != nullptr); 
        ASSERT_TRUE(field->wild_card->node_type == megrez::NT_WILDCARD);
    ...
    }
    {
        ...
        ASSERT_TRUE(select_stmt->fields[0]->node_type == megrez::NT_SELECT_FEILD);
        ...
        ASSERT_TRUE(select_stmt->fields[0]->wild_card->node_type == megrez::NT_WILDCARD);
        ...
        ASSERT_TRUE(select_stmt->fields[1]->node_type == megrez::NT_SELECT_FEILD);
        ...
        ASSERT_TRUE(select_stmt->fields[1]->wild_card->node_type == megrez::NT_WILDCARD);
        ...
        ASSERT_TRUE(select_stmt->fields[2]->node_type == megrez::NT_SELECT_FEILD);
        ...
        ASSERT_TRUE(select_stmt->fields[3]->node_type == megrez::NT_SELECT_FEILD);
        ...
        ASSERT_TRUE(select_stmt->fields[4]->node_type == megrez::NT_SELECT_FEILD);
...
TEST(test_parser, case_orderby) {
    ...
    // test select_orderby
    {
        ...
        ASSERT_EQ(5, select_stmt->fields.size());
        ASSERT_TRUE(select_stmt->order != nullptr);
        megrez::OrderByClause* order_by = select_stmt->order;
        ASSERT_EQ(1, order_by->items.size());
        ASSERT_TRUE(order_by->node_type == megrez::NT_ORDER_BY);
        megrez::ByItem* by_item = order_by->items[0];
    ...
    } 
    {
        ...
        ASSERT_TRUE(order_by->node_type == megrez::NT_ORDER_BY);
        megrez::ByItem* by_item_0 = order_by->items[0];
        ASSERT_TRUE(by_item_0->expr != nullptr);
        ASSERT_TRUE(by_item_0->is_desc == false);
        megrez::ByItem* by_item_1 = order_by->items[1];
...
TEST(test_parser, case_limit) {
    ...
    }
    {
        ...
        ASSERT_TRUE(by_item_1->is_desc == true);
        ASSERT_TRUE(select_stmt->limit != nullptr);
        megrez::LimitClause* limit = select_stmt->limit;
        std::cout << static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val << "aaa\n";
        int64_t count = static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val;
        int64_t offset = static_cast<megrez::LiteralExpr*>(limit->offset)->_u.int64_val;
        ASSERT_EQ(10, count);
        ASSERT_EQ(0, offset);
    }
    {
        ...
        ASSERT_TRUE(select_stmt->limit != nullptr);
        megrez::LimitClause* limit = select_stmt->limit;
        ASSERT_EQ(100, static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val);
        ASSERT_EQ(10, static_cast<megrez::LiteralExpr*>(limit->offset)->_u.int64_val);
    }
}
...
TEST(test_parser, case_lock) {
    //test select_lock
    {
        ...
        ASSERT_EQ(100, static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val);
        ASSERT_EQ(10, static_cast<megrez::LiteralExpr*>(limit->offset)->_u.int64_val);
        ASSERT_TRUE(select_stmt->lock == megrez::SL_NONE);
    }
    {
        megrez::SqlParser parser;
        ...
        ASSERT_TRUE(by_item_1->is_desc == true);
        ASSERT_TRUE(select_stmt->limit != nullptr);
        ASSERT_TRUE(select_stmt->lock == megrez::SL_FOR_UPDATE);
    }
    {
        megrez::SqlParser parser;
        ...
        ASSERT_TRUE(by_item_1->is_desc == true);
        ASSERT_TRUE(select_stmt->limit != nullptr);
        ASSERT_TRUE(select_stmt->lock == megrez::SL_IN_SHARE);
    }
}
...
TEST(test_parser, case_group) {
    {
        ...
        megrez::GroupByClause* group = select_stmt->group;
...
// test select_from
    {
        ...
        ASSERT_TRUE(typeid(*(select_stmt->table_refs)) == typeid(megrez::TableSource));
        megrez::TableSource* table_source = (megrez::TableSource*)select_stmt->table_refs;
        ...
        megrez::TableName* table_name = table_source->table_name;
    ...
    }
    {
        ...
        megrez::IndexHint* index_hint = table_source->index_hints[0];
        ASSERT_EQ(megrez::IHT_HINT_USE, index_hint->hint_type);
        ASSERT_EQ(megrez::IHS_HINT_SCAN, index_hint->hint_scope);
    ...
    }
    {
        ...
        megrez::IndexHint* index_hint = table_source->index_hints[0];
        ASSERT_EQ(megrez::IHT_HINT_USE, index_hint->hint_type);
        ASSERT_EQ(megrez::IHS_HINT_JOIN, index_hint->hint_scope);
    ...
    }
    {
        ...
        ASSERT_EQ(1, table_source->index_hints.size());
        megrez::IndexHint* index_hint = table_source->index_hints[0];
        ASSERT_EQ(megrez::IHT_HINT_IGNORE, index_hint->hint_type);
        ASSERT_EQ(megrez::IHS_HINT_ORDER_BY, index_hint->hint_scope);
    ...
    }
    {
        ...
        ASSERT_EQ(1, table_source->index_hints.size());
        megrez::IndexHint* index_hint = table_source->index_hints[0];
        ASSERT_EQ(megrez::IHT_HINT_FORCE, index_hint->hint_type);
        ASSERT_EQ(megrez::IHS_HINT_SCAN, index_hint->hint_scope);
        ASSERT_EQ(2, index_hint->index_name_list.size());
        ASSERT_EQ(std::string(index_hint->index_name_list[0].value), "uniq_key");
        ASSERT_EQ(std::string(index_hint->index_name_list[1].value), "primary");
...

    {
        ...
        ASSERT_TRUE(select_stmt->having == nullptr);
        ASSERT_TRUE(select_stmt->table_refs != nullptr);
        ASSERT_TRUE(typeid(*(select_stmt->table_refs)) == typeid(megrez::JoinNode));
        megrez::JoinNode* join_node = (megrez::JoinNode*)select_stmt->table_refs;
        ASSERT_EQ(megrez::NT_JOIN, join_node->node_type);
        ASSERT_EQ(megrez::JT_INNER_JOIN, join_node->join_type);
        ...
        ASSERT_TRUE(typeid(*(join_node->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(join_node->right)) == typeid(megrez::TableSource));
        megrez::TableSource* left = (megrez::TableSource*)join_node->left;
        megrez::TableSource* right = (megrez::TableSource*)join_node->right;
    ...
    }
    {
        ...
        megrez::ColumnName* column_name = join_node->using_col[0];
    ...
    }
    {
        ...
        megrez::ColumnName* column_name0 = join_node->using_col[0];
        ...
        megrez::ColumnName* column_name1 = join_node->using_col[1];
...

    {
        ...
        megrez::JoinNode* join_node = (megrez::JoinNode*)select_stmt->table_refs;
        ASSERT_EQ(megrez::NT_JOIN, join_node->node_type);
        ASSERT_EQ(megrez::JT_LEFT_JOIN, join_node->join_type);
        ASSERT_FALSE(join_node->is_natural);
        ASSERT_FALSE(join_node->is_straight);
        ASSERT_EQ(0, join_node->using_col.size());
        ASSERT_TRUE(join_node->expr != nullptr);
    ...
    }
    {
        ...
        megrez::JoinNode* join_node = (megrez::JoinNode*)select_stmt->table_refs;
        ASSERT_EQ(megrez::NT_JOIN, join_node->node_type);
        ASSERT_EQ(megrez::JT_RIGHT_JOIN, join_node->join_type);
        ASSERT_FALSE(join_node->is_natural);
        ASSERT_FALSE(join_node->is_straight);
        ASSERT_EQ(0, join_node->using_col.size());
        ASSERT_TRUE(join_node->expr != nullptr);
    ...
    }
    {
        ...
        ASSERT_TRUE(join_node->left != nullptr);
        ASSERT_TRUE(join_node->right != nullptr);
        ASSERT_TRUE(typeid(*(join_node->left)) == typeid(megrez::JoinNode));
        ASSERT_TRUE(typeid(*(join_node->right)) == typeid(megrez::TableSource));
        megrez::TableSource* right = (megrez::TableSource*)join_node->right;
        ASSERT_TRUE(right->table_name != nullptr);
        ASSERT_TRUE(std::string(right->table_name->db.value) == "db");
        ...
        megrez::JoinNode* left_join_node = (megrez::JoinNode*)join_node->left;
        std::cout << "left_join_node: " << left_join_node->to_string() << std::endl;
        ASSERT_EQ(megrez::NT_JOIN, left_join_node->node_type);
        ASSERT_EQ(megrez::JT_LEFT_JOIN, left_join_node->join_type);
        ...
        ASSERT_TRUE(typeid(*(left_join_node->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(left_join_node->right)) == typeid(megrez::TableSource));
        megrez::TableSource* left_join_node_right = (megrez::TableSource*)left_join_node->right;
        ...
        megrez::TableSource* left_join_node_left = (megrez::TableSource*)left_join_node->left;
...

    {
        ...
        ASSERT_TRUE(join_node->right != nullptr);
        ASSERT_TRUE(typeid(*(join_node->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(join_node->right)) == typeid(megrez::JoinNode));
        megrez::TableSource* left = (megrez::TableSource*)join_node->left;
        ASSERT_TRUE(left->table_name != nullptr);
        ASSERT_TRUE(std::string(left->table_name->db.value) == "db");
        ASSERT_TRUE(std::string(left->table_name->table.value) == "table_a");
        ...
        megrez::JoinNode* right_join_node = (megrez::JoinNode*)join_node->right;
        std::cout << "right_join_node: " << right_join_node->to_string() << std::endl;
        ASSERT_EQ(megrez::NT_JOIN, right_join_node->node_type);
        ASSERT_EQ(megrez::JT_LEFT_JOIN, right_join_node->join_type);
        ...
        ASSERT_TRUE(typeid(*(right_join_node->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(right_join_node->right)) == typeid(megrez::TableSource));
        megrez::TableSource* right_join_node_right = (megrez::TableSource*)right_join_node->right;
        ...
        megrez::TableSource* right_join_node_left = (megrez::TableSource*)right_join_node->left;
    ...
    }
    {
        ...
        megrez::JoinNode* left = (megrez::JoinNode*)join_node->left;
        ASSERT_EQ(megrez::NT_JOIN, left->node_type);
        ASSERT_EQ(megrez::JT_INNER_JOIN, left->join_type);
        ...
        ASSERT_TRUE(typeid(*(left->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(left->right)) == typeid(megrez::TableSource));
        megrez::TableSource* left_right = (megrez::TableSource*)left->right;
        ...
        megrez::TableSource* left_left = (megrez::TableSource*)left->left;
    ...
    }
    {
        ...
        ASSERT_TRUE(typeid(*(join_node->left)) == typeid(megrez::JoinNode));
        ASSERT_TRUE(typeid(*(join_node->right)) == typeid(megrez::JoinNode));
        megrez::JoinNode* right = (megrez::JoinNode*)join_node->right;
        ASSERT_EQ(megrez::NT_JOIN, right->node_type);
        ASSERT_EQ(megrez::JT_INNER_JOIN, right->join_type);
        ASSERT_TRUE(typeid(*(right->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(right->right)) == typeid(megrez::TableSource));
        megrez::TableSource* right_left = (megrez::TableSource*)right->left;
        megrez::TableSource* right_right = (megrez::TableSource*)right->right;
        ...
        megrez::JoinNode* left = (megrez::JoinNode*)join_node->left;
        ASSERT_EQ(megrez::NT_JOIN, left->node_type);
        ASSERT_EQ(megrez::JT_LEFT_JOIN, left->join_type);
        ASSERT_FALSE(left->is_natural);
        ASSERT_FALSE(left->is_straight);
        ASSERT_EQ(0, left->using_col.size());
        ASSERT_TRUE(left->expr != nullptr);
        ...
        ASSERT_TRUE(left->right != nullptr);
        ASSERT_TRUE(typeid(*(left->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(left->right)) == typeid(megrez::JoinNode));
        megrez::TableSource* left_left = (megrez::TableSource*)left->left;
        ASSERT_TRUE(left_left->table_name != nullptr);
        ASSERT_TRUE(std::string(left_left->table_name->db.value) == "db");
        ASSERT_TRUE(std::string(left_left->table_name->table.value) == "table_a");
        ...
        megrez::JoinNode* left_right = (megrez::JoinNode*)left->right;
        ASSERT_EQ(megrez::NT_JOIN, left_right->node_type);
        ASSERT_EQ(megrez::JT_LEFT_JOIN, left_right->join_type);
        ...
        ASSERT_TRUE(typeid(*(left_right->left)) == typeid(megrez::TableSource));
        ASSERT_TRUE(typeid(*(left_right->right)) == typeid(megrez::TableSource));
        megrez::TableSource* left_right_left = (megrez::TableSource*)left_right->left;
        ...
        megrez::TableSource* left_right_right = (megrez::TableSource*)left_right->right;
...
TEST(test_parser, case_union) {
    //test union clause
    {
        ...
        ASSERT_EQ(0, parser.error);
        ASSERT_EQ(1, parser.result.size());
        ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(megrez::UnionStmt));
        megrez::UnionStmt* union_stmt = (megrez::UnionStmt*)parser.result[0];
        std::cout << union_stmt->to_string() << std::endl;
        ASSERT_EQ(2, union_stmt->select_stmts.size());
        ASSERT_TRUE(typeid(*(union_stmt->select_stmts[0])) == typeid(megrez::SelectStmt));
        ASSERT_TRUE(typeid(*(union_stmt->select_stmts[1])) == typeid(megrez::SelectStmt));
        megrez::SelectStmt* select_stmt1 = (megrez::SelectStmt*)union_stmt->select_stmts[0];
        megrez::SelectStmt* select_stmt2 = (megrez::SelectStmt*)union_stmt->select_stmts[1];
    ...
    }
    {
        ...
        ASSERT_TRUE(select_stmt2->limit == nullptr);
        ASSERT_TRUE(union_stmt->limit != nullptr);
        megrez::LimitClause* limit = union_stmt->limit;
        std::cout << static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val << "\n";
        int64_t count = static_cast<megrez::LiteralExpr*>(limit->count)->_u.int64_val;
        int64_t offset = static_cast<megrez::LiteralExpr*>(limit->offset)->_u.int64_val;
        ASSERT_EQ(10, count);
        ASSERT_EQ(0, offset);
        ASSERT_TRUE(union_stmt->order != nullptr);
        megrez::OrderByClause* order_by = union_stmt->order;
        ASSERT_EQ(1, order_by->items.size());
        ASSERT_TRUE(order_by->node_type == megrez::NT_ORDER_BY);
        megrez::ByItem* by_item = order_by->items[0];
        ASSERT_TRUE(by_item->expr != nullptr);
    ...
    }
    {
        ...
        ASSERT_TRUE(typeid(*(union_stmt->select_stmts[0])) == typeid(megrez::SelectStmt));
        ASSERT_TRUE(typeid(*(union_stmt->select_stmts[1])) == typeid(megrez::SelectStmt));
        ASSERT_TRUE(typeid(*(union_stmt->select_stmts[2])) == typeid(megrez::SelectStmt));
        megrez::SelectStmt* select_stmt1 = (megrez::SelectStmt*)union_stmt->select_stmts[0];
        megrez::SelectStmt* select_stmt2 = (megrez::SelectStmt*)union_stmt->select_stmts[1];
        megrez::SelectStmt* select_stmt3 = (megrez::SelectStmt*)union_stmt->select_stmts[2];
        ASSERT_TRUE(select_stmt1->order != nullptr);
        ASSERT_TRUE(select_stmt1->limit != nullptr);
        ASSERT_TRUE(select_stmt2->order != nullptr);
        ASSERT_TRUE(select_stmt2->limit != nullptr);
...
TEST(test_parser, case_from_subselect) {
    //test union clause
    {
        ...
        ASSERT_TRUE(typeid(*(table_source->derived_table)) == typeid(megrez::SelectStmt));
        megrez::SelectStmt* select_stmt1 = (megrez::SelectStmt*)table_source->derived_table;
...
TEST(test_parser, case_exists_subselect) {
    //test exists subquery clause
    {
        ...
        std::cout << select_stmt->to_string() << std::endl;
        ASSERT_TRUE(select_stmt->table_refs != nullptr);
        ASSERT_TRUE(select_stmt->where->expr_type == megrez::ET_EXISTS_SUB_QUERY_EXPR);
        megrez::ExistsSubqueryExpr* table_where = (megrez::ExistsSubqueryExpr*)select_stmt->where;
        ASSERT_TRUE(typeid(*(table_where->query_expr)) == typeid(megrez::SubqueryExpr));
        megrez::SubqueryExpr* query_expr = (megrez::SubqueryExpr*)table_where->query_expr;
        megrez::SelectStmt* select_stmt1 = (megrez::SelectStmt*)(query_expr->query_stmt);
        ASSERT_TRUE(select_stmt1->fields.size() == 2);
        ASSERT_TRUE(select_stmt->fields.size() == 2);
        ASSERT_TRUE(select_stmt->order != nullptr);
        ASSERT_TRUE(select_stmt->limit != nullptr);
    }
    {
        ...
        std::cout << select_stmt->to_string() << std::endl;
        ASSERT_TRUE(select_stmt->table_refs != nullptr);
        megrez::ExprNode* exists_expr = (megrez::ExprNode*)(select_stmt->where->children[0]);
        ASSERT_TRUE(exists_expr->expr_type == megrez::ET_EXISTS_SUB_QUERY_EXPR);
        megrez::ExistsSubqueryExpr* sub_expr = (megrez::ExistsSubqueryExpr*)exists_expr;
        ASSERT_TRUE(typeid(*(sub_expr->query_expr)) == typeid(megrez::SubqueryExpr));
        megrez::SelectStmt* select_stmt1 = (megrez::SelectStmt*)(sub_expr->query_expr->query_stmt);
        ASSERT_TRUE(select_stmt1->fields.size() == 2);
        ASSERT_TRUE(select_stmt->fields.size() == 2);
        ASSERT_TRUE(select_stmt->order != nullptr);
        ASSERT_TRUE(select_stmt->limit != nullptr);
...
TEST(test_parser, case_anyorall_subselect) {
    //test anyorall subquery clause
    {
        ...
        std::cout << select_stmt->to_string() << std::endl;
        ASSERT_TRUE(select_stmt->table_refs != nullptr);
        ASSERT_TRUE(typeid(*(select_stmt->where)) == typeid(megrez::CompareSubqueryExpr));
        megrez::CompareSubqueryExpr* sub_expr = (megrez::CompareSubqueryExpr*)select_stmt->where;
