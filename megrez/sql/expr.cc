...
#include <megrez/sql/expr.h>
#include <megrez/sql/dml.h>
#include <unordered_map>
#include <turbo/flags/flag.h>

TURBO_FLAG(bool, normalize_case_when_to_stream, false, "whether reduce case_when/case_expr_when func's children to 2 when to_stream");
namespace megrez {
...
FuncExpr* FuncExpr::new_unary_op_node(FuncType t, Node* arg1, turbo::Arena& arena) {
...
FuncExpr* FuncExpr::new_binary_op_node(FuncType t, Node* arg1, Node* arg2, turbo::Arena& arena) {
    FuncExpr* fun = new(arena.allocate(sizeof(FuncExpr)))FuncExpr();
    fun->func_type = t;
    fun->fn_name = type_to_name(t);
    fun->children.push_back(arg1, arena);
...
FuncExpr* FuncExpr::new_ternary_op_node(
        FuncType t, Node* arg1, Node* arg2, Node* arg3, turbo::Arena& arena) {
    FuncExpr* fun = new(arena.allocate(sizeof(FuncExpr)))FuncExpr();
    fun->func_type = t;
    fun->fn_name = type_to_name(t);
...
void SubqueryExpr::to_stream(std::ostream& os) const {
    os << "(";
    if (query_stmt != nullptr && query_stmt->node_type == megrez::NT_SELECT) {
        megrez::SelectStmt* select_stmt = (megrez::SelectStmt*)query_stmt;
        select_stmt->to_stream(os);
    } else if (query_stmt != nullptr && query_stmt->node_type == megrez::NT_UNION) {
        megrez::UnionStmt* union_stmt = (megrez::UnionStmt*)query_stmt;
