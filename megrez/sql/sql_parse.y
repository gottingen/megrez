...
using megrez::SqlParser;
using megrez::InsertStmt;
using megrez::Node;
using megrez::Assignment;
using megrez::ExprNode;
using megrez::FuncType;
using megrez::FuncExpr;
using megrez::StmtNode;
using megrez::String;
using megrez::RowExpr;
using megrez::ColumnName;
using megrez::TableName;
using megrez::PriorityEnum;
using megrez::CreateTableStmt;
...
LimitClause:
    ...
    | LIMIT SimpleExpr {
        if ($2->expr_type != megrez::ET_LITETAL) {
            sql_error(&@2, yyscanner, parser, "limit expr only support INT_LIT or PLACE_HOLDER");
            return -1;
        }
        LiteralExpr* count = (LiteralExpr*)$2;
        if (count->literal_type != megrez::LT_INT && count->literal_type != megrez::LT_PLACE_HOLDER) {
            sql_error(&@2, yyscanner, parser, "limit expr only support INT_LIT or PLACE_HOLDER");
            return -1;
        }
    ...
    | LIMIT SimpleExpr ',' SimpleExpr {
        ...
        if (offset->literal_type != megrez::LT_INT && offset->literal_type != megrez::LT_PLACE_HOLDER) {
            sql_error(&@2, yyscanner, parser, "limit expr only support INT_LIT or PLACE_HOLDER");
            return -1;
        }

        if ($4->expr_type != megrez::ET_LITETAL) {
            sql_error(&@2, yyscanner, parser, "limit expr only support INT_LIT or PLACE_HOLDER");
            return -1;
        }
...
CompareSubqueryExpr:
    Expr CompareOp AnyOrAll SubSelect %prec EQ_OP {
        ...
        comp_sub_query->cmp_type = (megrez::CompareType)$3;
...
SignedLiteral:
    ...
    | '-' NumLiteral
    {
        ...
        if (literal->literal_type == megrez::LT_INT) {
...
int sql_error(YYLTYPE* yylloc, yyscan_t yyscanner, SqlParser *parser, const char *s) {    
    parser->error = megrez::SYNTAX_ERROR;
