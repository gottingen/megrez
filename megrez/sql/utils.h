...
struct String {
    char* value;
    size_t length;
    void strdup(const char* str, int len, turbo::Arena& arena) {
        value = (char*)arena.allocate(len + 1);
        length = len;
        memcpy(value, str, len);
        value[len] = '\0';
    }
    void strdup(const char* str, turbo::Arena& arena) {
        strdup(str, strlen(str), arena);
    }
    void append(const char* str, turbo::Arena& arena) {
        int len = strlen(str);
        int old_len = length;
        length += len;
...
public:
    ...
    void push_back(const T& value, turbo::Arena& arena) {
        if (remain() > 0) {
            *_end = value;
            ++_end;
        } else {
